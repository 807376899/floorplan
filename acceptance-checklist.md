...
- [ ] 已有实验室详情不显示“分配状态”和“生效时间”；admin 在可编辑方案中可从详情栏内联编辑实验室和改建房间，改建保存后旧分配变为 `Invalid`，当前空间绑定新的“待改建房间”用途单元，改建年月写入当前分配的 `effective_from`。
- [ ] 主图默认适配显示大小。
- [ ] 主图默认居中显示。
...
- [ ] 业务编辑保存、刷新渲染或重新选择空间后，当前选中空间卡片在左侧空间列表可见，不会因为滚动位置让用户误判空间消失。
...
- [ ] 普通“规划”未规划空间入口仍位于业务编辑实验室信息区；admin 在可编辑方案中可从详情栏内联编辑已有实验室、改建当前房间，并可在“更多”菜单中编辑当前房间资料。
- [ ] 详情栏“编辑房间”可维护门牌、骨架、所在侧、偏移、长宽、面积、网段和物理状态；修改门牌后空间编码引用同步迁移，已有房间长宽不可清空，长宽有效时面积按乘积计算。
- [ ] 详情栏“更多”菜单中的合并房间和拆分房间入口在功能实现前保持禁用并标注暂未开放。
