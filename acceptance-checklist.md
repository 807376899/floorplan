...
- [ ] 本机直接运行 `npm start` 或 `node server.js` 默认监听 `3300`，遇到 Windows 保留端口或端口占用时输出可执行的端口切换提示，且提示中的示例端口不等于刚失败的端口。
...
- [ ] 系统不预设特定学校的校区名称；空库或旧库可没有 `campuses` 关系行，admin 通过校区基础表/数据库维护后才出现真实校区选项。
- [ ] 教学楼基础表的所属校区使用 `campus_code` 下拉框，选项来自校区字典；没有校区字典时只显示“未设置校区”，不得把楼号编码推导出的校区前缀当作真实选项。
- [ ] 数据库 `buildings.campus_code` 支持教学楼-校区联动：非空编码必须引用已存在校区，删除仍被教学楼引用的校区会失败，修改校区编码/名称会同步迁移教学楼引用，投影和原始维护保存不再输出或写入 `campus_zone`。
