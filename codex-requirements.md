...
- 本机直接运行 `npm start` 或 `node server.js` 时，默认端口必须避开 Windows 常见保留端口范围，当前默认使用 `3300`；需要使用其他端口时通过 `PORT` 环境变量显式指定。
- 服务端监听失败时，`EACCES` 和 `EADDRINUSE` 必须输出可执行的端口切换提示，不应只抛出 Node 原始堆栈；提示中的示例端口不得与刚失败的端口相同。
...
- Once relational business rows exist, legacy active/copy JSON must not filter or hide relational `spaces` or `labs`, and must not be used as the source of truth for active detail actions, explicit numbering normalization, managed-plan previews, or creating a new copy from an existing copy. These paths must project the target active plan or copy from relation tables first, using legacy JSON only when no corresponding relation plan exists yet.
- When a copy plan code changes during explicit numbering normalization, the relation-table plan identity and child rows must migrate without violating `plans.copy_id` uniqueness; stale copy `dataset_json`, `plan_json`, or `assignments_json` must not recreate old plan codes, stale spaces, stale labs, or stale assignments.
- Creating a new copy from an existing copy must copy the source copy's relation-table space/lab overrides, assignments, and scoped deleted-space tombstones into the new copy scope. Stale legacy payload rows from the source copy must not replace relation-table differences.
...
- Campus names are not system presets. Empty deployments and legacy databases may have no `campuses` rows; admins add campus dictionary rows through the database/raw maintenance path, and building-campus linkage must remain reusable across schools.
- Teaching buildings must store the real campus relationship in `buildings.campus_code` when a matching campus dictionary row exists. SQLite schema must enforce valid non-empty campus codes and block deleting a campus referenced by buildings; UI normalization must not treat a building-code-derived campus prefix as a real campus relationship.
- Editing campus rows through raw maintenance must migrate referenced `buildings.campus_code` and display `campus_zone` atomically before deleting old campus codes, so changing a campus code/name does not orphan existing teaching buildings.
