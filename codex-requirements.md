...
- 已有实验室的详细信息栏不得展示“分配状态”和“生效时间”；admin 在可编辑方案中可从详细信息栏内联执行“编辑实验室”和“改建房间”。改建房间保存后，原实验室与空间解绑，原分配标记为 `Invalid`，并为该空间创建和绑定新的用途单元；新用途单元默认名称为“待改建房间”，改建年月写入当前方案分配的 `effective_from`。
...
- 业务编辑保存、刷新渲染或重新选择空间后，当前选中空间卡片必须保持在左侧空间列表可见范围内，避免新增或编辑后的空间因列表滚动位置而看起来消失。
...
- 普通“规划”未规划空间操作仍位于业务编辑的实验室信息区；admin 在可编辑方案中可在详情信息栏内联编辑已有实验室资料、改建当前房间，以及通过“更多”菜单编辑当前空间资料。详情栏编辑房间可维护门牌、骨架、所在侧、偏移、长宽、面积、网段和物理状态；门牌修改必须预览并迁移空间编码引用，已有房间长宽不得被清空，长宽有效时面积按长宽乘积计算。“更多”菜单中的合并房间和拆分房间入口在功能实现前必须保持禁用并标注暂未开放。
