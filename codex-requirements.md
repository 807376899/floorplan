...
- Teaching buildings must store the real campus relationship only in `buildings.campus_code` when a matching campus dictionary row exists. `campus_zone` is a legacy import/migration input only and must not be created as a new SQLite building column, returned in projected building rows, or saved from raw maintenance/UI edits.
- Editing campus rows through raw maintenance must migrate referenced `buildings.campus_code` atomically before deleting old campus codes, so changing a campus code/name does not orphan existing teaching buildings.
