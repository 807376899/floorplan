...
- 未登录用户只能浏览共享基线数据和已公开的方案副本；界面不会显示“上传数据包”，后端上传接口也只允许 `admin` 调用。
- 登录用户可以基于当前方案创建自己的方案副本，编辑该副本中的方案分配和搬迁结果，并选择将副本设为公开或私有。
- 不同用户可以创建相同名称的方案副本；公开后匿名视图使用“公开 #ID”进行区分，不展示创建者身份。
- 登录用户只能修改或删除自己创建的方案副本；`admin` 可以管理所有副本，但他人副本会显示为“用户名 的副本”，不会误标成“我的副本”。
- 同一副本保存时使用副本级 `revision` 做冲突检测，避免旧页面覆盖新修改。
- `admin` 可以维护共享基线数据、上传 `.xlsx` / `.json` 数据包、管理导入草稿、管理用户、恢复快照和修复中文显示。
...
- `admin`：管理员。拥有 `editor` 能力，并可维护共享基线、上传数据包、管理导入草稿、管理用户、管理快照和执行维护操作。

多人可以同时在线编辑各自创建的方案副本。当前实现不提供多人实时共同编辑同一个副本；如果同一个副本被多个页面同时保存，后保存的一方会收到版本冲突提示。

## 用户管理

`admin` 登录后可以在界面中打开“管理用户”：

- 查看用户列表、角色和启用状态。
- 创建 `admin` 或 `editor` 用户。
- 禁用用户。禁用是软删除：用户记录保留，账号不能再登录，现有登录 session 会被撤销。

禁用用户不会删除该用户创建的历史方案副本；已经公开的副本保持公开状态。
